                ...
                |> Seq.map (fun (coinId, trades) ->
                    ...
                    let currency =
                        if isMultiCurrency then
                            ...
                            (trades |> Seq.head).AmountPaid.Currency
                    ...
                    let singleCurrencyFolder accumulator currentTrade =
                        ...
                        | TradeType.Buy -> (fst accumulator + currentTrade.Amount), (snd accumulator + (amountPaid))
                        | TradeType.Sell -> (fst accumulator - currentTrade.Amount), (snd accumulator - (amountPaid))
                        | TradeType.Withdrawal -> accumulator
                    ...
                    let properties =
                        [ if trades
                             |> Seq.exists (fun t -> t.Type = Withdrawal) then
                            yield Property.HasExternalAmount

                          if isMultiCurrency then yield Property.IsMultiCurrency ]

                    { Asset.Cryptocoin =
                          (trades
                           |> Seq.find (fun t -> t.Cryptocoin.Id = coinId))
                              .Cryptocoin
